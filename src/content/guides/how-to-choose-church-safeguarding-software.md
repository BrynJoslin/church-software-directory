---
title: "How to choose church safeguarding software"
...
The Charity Commission's trustee guidance calls for suitable policies and practices, appropriate checks, and a clear system for referral or reporting. That is the standard against which a tool should be tested; a dashboard is not the standard.[^charity-detail]

## Define the information boundary and access model

Safeguarding records may contain information that is particularly sensitive. The ICO explains that special-category data has additional rules and that organisations should consider data minimisation and security alongside their lawful basis and condition for processing.[^ico-special] Do not treat a supplier's general GDPR statement as the answer for your church's actual use.
...
1. State the problem the product will solve and the processes it will not replace.
2. Record evidence seen, unresolved questions and the advice received.
3. Agree account owners, training, review dates and the escalation route if the system fails.
4. Agree a data-migration boundary; migrate only material approved under the church's records approach.
5. Set a post-launch review date before signing the contract.
...
- [iKnow Safeguarding](/software/iknow-safeguarding/) is a dedicated safeguarding profile to investigate where controlled case administration is the stated need.
- [Parish Safeguarding Dashboard](/software/parish-safeguarding-dashboard/) is relevant to Church of England contexts; confirm current diocesan fit and local procedures.
- [MyConcern](/software/tes-myconcern/) is a broader safeguarding-recording product; establish whether its workflows fit a church's operating boundary.
- [ChurchSuite](/software/churchsuite/) may be relevant where the church already uses its wider database, but a connected system is not evidence that every safeguarding requirement is met.
